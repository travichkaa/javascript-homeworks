// задача 1
function calculateReputation(countLikes, countDislikes, countEditedMessages, countMessages) {
    if (countMessages === 0) {
        return 0;
    }
    ...
    return (countLikes - countDislikes) * ((countMessages - countEditedMessages) / countMessages);
}

let reputation = calculateReputation(10, 15, 10, 20);
console.log(reputation);

// задача 2
function printThreadMessages(thread, countMessages) {
    if (thread.messages.length === 0) {
        console.log('Ошибка формата! В теме нет сообщений');
        return;
    }

    const countMessagesToPrint = Math.min(countMessages, thread.messages.length);

    const messagesToPrint = thread.messages.slice(-countMessagesToPrint);

    for (let message of messagesToPrint) {
...
printThreadMessages(thread, 3);

// задача 3
function returnStatistics(messages) {
    const countMessages = messages.length;
    let countEditedMessages = 0;
    let countNotEditedMessages = 0;
    for (let message of messages) {
        if (message.edited) {
            countEditedMessages++;
            continue;
        }
        countNotEditedMessages++;
    }

    return {
        messages: countMessages,
        editedMessages: countEditedMessages,
        notEditedMessages: countNotEditedMessages
    };
}
...
let statistics = returnStatistics(allMessages);
